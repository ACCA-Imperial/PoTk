...
methods(Hidden) % Documentation
    ...
    function str = latexExpression(U)
        str = ['U \left( \overline{\mu} e^{i\chi} ', ...
            '\frac{\partial G_0(\zeta;\alpha,\overline{\alpha})}', ...
            '{\partial\overline{\alpha}} - \mu e^{-i\chi} ', ...
            '\frac{\partial G_0(\zeta;\alpha,\overline{\alpha})}', ...
            '{\partial\alpha} \right)', ...
            ' \qquad\mathrm{(uniform\;flow)}'];
    end
    
    function uniformFlowTermLatexToDoc(~, do)
        do.addln(...
            ['the uniform flow strength ', do.eqInline('U'), ...
            ', the flow angle ', do.eqInline('\chi'), ...
            ' and the residual ', do.eqInline('\mu'), ...
            ' of the conformal map ', do.eqInline('z(\zeta)')
            ])
    end
end
