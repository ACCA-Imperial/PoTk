...
methods(Hidden)
    ...
    function str = latexExpression(~)
        str = [...
            '2\pi U_f b\mathrm{i} \left.\left( e^{\mathrm{i}\chi} ', ...
    ...
    function uniformFlowTermLatexToDoc(~, do)
        do.addln(...
            ...
            do.eqInline('U_f'), ' at angle ', ...
            do.eqInline('\chi')])
        do.addln()
        do.addln(...
            ['the scalar ', do.eqInline('b'), ...
            ' by the form of the map ', ...
            do.eqInline('z(\zeta)'), ' from the unit domain to the ', ...
            'physical domain as ', ...
            do.eqInline('\zeta\to\beta'), ' such that'])
